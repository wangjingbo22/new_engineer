...
pattern = r'if \(rc\.s\[0\] == 1 && rc\.s\[1\] == 1\)\n      \{\n        // \[原逻辑\] s\[0\]=1 且 s\[1\]=1：控制大小臂'
...
if count > 0:
    ...
    print("Failed to inject sync for arm")
    
pattern = r'else if \(rc\.s\[0\] == 2 && rc\.s\[1\] == 2\)\n      \{\n        // \[新逻辑\] s\[0\]=2 且 s\[1\]=2：控制末端夹爪'
...
if count > 0:
    ...
    print("Failed to inject sync for claw")

pattern = r'osDelay\(2\); // 500Hz control loop\n    \}'
...
if count > 0:
    ...
    print("Failed to inject state tracking")
